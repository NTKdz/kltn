...
class DeepQLearningAgent:
    ...
    def replay(self):
        ...
        individual_rewards = torch.FloatTensor(
            individual_rewards).to(self.device)
    ...
    def plot_progress(self, total_history, per_user_history, total_history_greedy=None, per_user_history_greedy=None,
                      total_history_sinr=None, per_user_history_sinr=None, filename="multi_user_training_progress_tdma_reverted.png"):
        plt.figure(figsize=(14, 10))
        plt.plot(total_history, label='DRL Total Avg Reward', color='blue')
        for u in range(self.num_users):
            plt.plot(
                per_user_history[u], label=f'DRL User {u+1} Avg Reward', linestyle='--', alpha=0.5)
        if total_history_greedy is not None and per_user_history_greedy is not None:
            plt.plot(total_history_greedy,
                     label='Greedy Total Avg Reward', color='red')
            for u in range(self.num_users):
                plt.plot(
                    per_user_history_greedy[u], label=f'Greedy User {u+1} Avg Reward', linestyle=':', alpha=0.5)
        if total_history_sinr is not None and per_user_history_sinr is not None:
            plt.plot(total_history_sinr,
                     label='SINR Total Avg Reward', color='green')
            for u in range(self.num_users):
                plt.plot(
                    per_user_history_sinr[u], label=f'SINR User {u+1} Avg Reward', linestyle='-.', alpha=0.5)
        plt.xlabel('Iteration (Time Slot)')
        plt.ylabel('Average Reward')
        plt.title('DRL vs Greedy vs SINR-Based Strategy Comparison (TDMA)')
        plt.legend()
        plt.grid(True)
    ...
    def log_to_file(self, filename, iteration, avg_total, avg_per_user, packet_loss_ratios, avg_losses, count_change, packet_lost_by_transmit):
        os.makedirs(os.path.dirname(filename), exist_ok=True)
        header = f"Iteration\tAvg_Total_Reward\t" + "\t".join([f"Avg_Reward_User_{u}" for u in range(self.num_users)]) + \
    ...
    def run_greedy_simulation(self, num_iterations=T, log_path="log/multi_user_training_data_tdma_reverted.txt"):
        env = Environment(self.num_users)
        total_reward = 0
        ...
        for i in range(num_iterations):
            ...
            current_agent = env.time_slot
            possible_actions = env.get_possible_actions(current_agent)
            current_action = env.greedy_action(current_agent)
            ...
            if current_action in transmission_actions:
                ...
                for u in range(self.num_users):
                    ...
                    possible_transmissions = [
                        a for a in env.get_possible_actions(u) if a in transmission_actions]
                    if possible_transmissions:
                        action = env.greedy_action(u)
                        if action in transmission_actions:
                            candidates.append((u, action))

                if candidates:
                    best_user, best_action = random.choice(candidates)
            ...
            total_reward_step, next_state, individual_rewards, packets_arrived, packets_lost, packets_lost_by_transmit = env.step(
                actions)
            ...
            if (i + 1) % step == 0:
                ...
                packet_loss_by_transmit_ratios = [total_packets_lost_by_transmit[u] / (total_packets_arrived[u] + total_packets_lost_by_transmit[u]) if total_packets_arrived[u] > 0 else 0
                                                  for u in range(self.num_users)]
                avg_loss_per_user = [0] * self.num_users
                print(f"Greedy Time Slot {i + 1}, Avg Total Reward: {avg_total:.4f}, "
                      ...
                      f"Count Change: {count_change}")
                self.log_to_file(log_path, i + 1, avg_total, avg_per_user, packet_loss_ratios,
                                 avg_loss_per_user, count_change, packet_loss_by_transmit_ratios, strategy="Greedy")
        ...
        avg_total = total_reward / num_iterations
        avg_per_user = [per_user_totals[u] /
                        num_iterations for u in range(self.num_users)]
        packet_loss_ratios = [total_packets_lost[u] / (total_packets_arrived[u] + total_packets_lost[u]) if total_packets_arrived[u] > 0 else 0
                              for u in range(self.num_users)]
        packet_loss_by_transmit_ratios = [total_packets_lost_by_transmit[u] / (total_packets_arrived[u] + total_packets_lost_by_transmit[u]) if total_packets_arrived[u] > 0 else 0
                                          for u in range(self.num_users)]
        self.log_to_file(log_path, num_iterations, avg_total, avg_per_user, packet_loss_ratios, [
                         0] * self.num_users, count_changes[-1] if count_changes else 0, packet_loss_by_transmit_ratios, strategy="Greedy")
        return avg_total, avg_per_user, packet_loss_ratios, packet_loss_by_transmit_ratios, total_history, per_user_history

    def run_sinr_simulation(self, num_iterations=T, log_path="log/multi_user_training_data_tdma_reverted_sinr1.txt"):
        env = Environment(self.num_users)
        total_reward = 0
        per_user_totals = [0] * self.num_users
        total_packets_arrived = [0] * self.num_users
        ...
        for i in range(num_iterations):
            ...
            env.current_d_bj = random.choices(d_bj_arr, nu_p, k=1)[0]
            env.current_e_hj = random.choices(e_hj_arr, nu_p, k=1)[0]
            env.current_d_ra = random.choices(dt_ra_arr, nu_p, k=1)[0]
            actions = [env.sinr_based_action(u) for u in range(self.num_users)]

            # actions = [env.random_action_strategy(u)
            #            for u in range(self.num_users)]
            ...
            if (i + 1) % step == 0:
                ...
                packet_loss_by_transmit_ratios = [total_packets_lost_by_transmit[u] / (total_packets_arrived[u] + total_packets_lost_by_transmit[u]) if total_packets_arrived[u] > 0 else 0
                                                  for u in range(self.num_users)]
                avg_loss_per_user = [0] * self.num_users
                print(f"SINR Time Slot {i + 1}, Avg Total Reward: {avg_total:.4f}, "
                      f"Avg Per-User Rewards: {[f'{r:.4f}' for r in avg_per_user]}, "
                      f"Packet Loss Ratios: {[f'{r:.4f}' for r in packet_loss_ratios]}, "
                      f"Packet Loss Ratios By Transmit: {[f'{r:.4f}' for r in packet_loss_by_transmit_ratios]}, "
                      f"Count Change: {count_change}")
                self.log_to_file(log_path, i + 1, avg_total, avg_per_user, packet_loss_ratios,
                                 avg_loss_per_user, count_change, packet_loss_by_transmit_ratios)
                count_changes.append(count_change)
                count_change = 0
                end = time.time()
                print(f"SINR Time: {end - start:.2f}s")
                start = end
        ...
        packet_loss_by_transmit_ratios = [total_packets_lost_by_transmit[u] / (total_packets_arrived[u] + total_packets_lost_by_transmit[u]) if total_packets_arrived[u] > 0 else 0
                                          for u in range(self.num_users)]
        self.log_to_file(log_path, num_iterations, avg_total, avg_per_user, packet_loss_ratios, [
                         0] * self.num_users, count_changes[-1] if count_changes else 0, packet_loss_by_transmit_ratios)
        return avg_total, avg_per_user, packet_loss_ratios, packet_loss_by_transmit_ratios, total_history, per_user_history
    ...
    def train(self, save_path="multi_user_checkpoint_tdma_reverted.pth",
        ...
        for i in range(T):
            ...
            if (i + 1) % step == 0:
                ...
                if losses is not None:
                    ...
                    print(f"DRL Time Slot {i + 1}, Avg Total Reward: {avg_total:.4f}, "
                          f"Avg Per-User Rewards: {[f'{r:.4f}' for r in avg_per_user]}, "
                ...
                self.log_to_file(log_path, i + 1, avg_total, avg_per_user, packet_loss_ratios,
                                 avg_loss_per_user, count_changes[-1], packet_loss_by_transmit_ratios, strategy="DRL")

                if (i + 1) == 100000:
                    self.plot_progress(total_history, per_user_history,
                                       filename=f"plot/test_final10/multi_user_progress_at_100k_tdma_reverted_{num_users}_rate_{arrival_rate}_eps_{self.epsilon_decay}_1.png")
        ...
        final_avg_loss_per_user = [np.mean(loss_history[u]) for u in range(
            self.num_users)] if loss_history[0] else [0] * self.num_users
        self.save_models(save_path)
        self.plot_progress(total_history, per_user_history, filename=plot_path)
        self.log_to_file(log_path, T, total_reward / T, avg_per_user_final,
                         final_packet_loss_ratios, final_avg_loss_per_user, count_changes[-1] if count_changes else 0, final_packet_loss_ratios_by_transmit)
        print(
        ...
        return total_reward / T, avg_per_user_final, total_history, per_user_history, final_packet_loss_ratios, final_packet_loss_ratios_by_transmit


if __name__ == "__main__":
    agent = DeepQLearningAgent(load_path=None)

    # Run SINR-based simulation
    avg_total_sinr, avg_per_user_sinr, packet_loss_ratios_sinr, packet_loss_by_transmit_sinr, total_history_sinr, per_user_history_sinr = agent.run_sinr_simulation(
        log_path=f"log/test_final10/multi_user_training_data_tdma_reverted_{num_users}_rate_{arrival_rate}_eps_{agent.epsilon_decay}_sinr1.txt"
    )
    print(f"SINR Multi-user total average reward: {avg_total_sinr:.4f}")
    print(
        f"SINR Multi-user per-user average rewards: {[f'{r:.4f}' for r in avg_per_user_sinr]}")
    print(
        f"SINR Multi-user average per-user reward (mean): {np.mean(avg_per_user_sinr):.4f}")
    print(
        f"SINR Packet Loss Ratios: {[f'{r:.4f}' for r in packet_loss_ratios_sinr]}")
    print(
        f"SINR Packet Loss Ratios By Transmit: {[f'{r:.4f}' for r in packet_loss_by_transmit_sinr]}")

    # Plot comparison
    agent.plot_progress(
        total_history_sinr, per_user_history_sinr,
        filename=f"plot/test_final10/drl_vs_greedy_vs_sinr_tdma_reverted_{num_users}_rate_{arrival_rate}_eps_{agent.epsilon_decay}_1.png"
    )
