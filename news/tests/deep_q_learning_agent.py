# deep_q_learning_agent_pytorch.py
...
class DeepQLearningAgent:
    ...
    def plot_progress(self, total_history, per_user_history, filename="multi_user_training_progress.png"):
        ...
        plt.title('Multi-User Training Progression')
        plt.legend()
        plt.grid(True)
    ...
    def train(self, save_path="multi_user_checkpoint.pth", plot_path="multi_user_training_progress.png"):
        total_reward = 0
        ...
        loss_history = [[] for _ in range(self.num_users)]  # Track loss history per user
        ...
        for i in range(T):
            ...
            if losses is not None:
                for u in range(self.num_users):
                    loss_history[u].append(losses[u])  # Append loss for each user
            state = next_state
            if (i + 1) % 10000 == 0:
                self.update_target()
            if (i + 1) % step == 0:
                ...
                if (i + 1) == 100000:
                    self.plot_progress(total_history, per_user_history, "plot/multi_user_progress_at_100k_0.png")
...
if __name__ == "__main__":
    agent = DeepQLearningAgent(load_path="checkpoint/multi_user_checkpoint_0.pth")
    avg_total_multi, avg_per_user_multi = agent.train(
        save_path="checkpoint/multi_user_checkpoint_0.pth",
        plot_path="plot/multi_user_training_progress_0.png"
    )
