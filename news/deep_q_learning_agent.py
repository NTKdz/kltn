    ...
    def train(self, save_path="multi_user_checkpoint.pth", plot_path="multi_user_training_progress.png"):
        ...
        for i in range(T):
            ...
            if (i + 1) % step == 0:
                ...
                if (i + 1) == 100000:
                    self.plot_progress(total_history, per_user_history, "multi_user_progress_at_100k.png")
        avg_per_user_final = [per_user_totals[u] / T for u in range(self.num_users)]
        self.save_models(save_path)
        # Plot and save after training
        self.plot_progress(total_history, per_user_history, plot_path)
        return total_reward / T, avg_per_user_final

if __name__ == "__main__":
    agent = DeepQLearningAgent(load_path="multi_user_checkpoint.pth")
    avg_total_multi, avg_per_user_multi = agent.train(
        save_path="multi_user_checkpoint.pth",
        plot_path="multi_user_training_progress.png"
    )
