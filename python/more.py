...
class Environment:
    ...
    def get_jammer_channel(self):
        ...
        if self.jammer_switch_counter >= self.jammer_switch_threshold:
            self.jammer_switch_counter = 0
            # Sweep jammer moves to next channel
...
def run_simulation(num_users=2, num_channels=5, num_episodes=10000):
    ...
    return normalized_rates, initial_actions, convergent_actions
...
def sensing_based_method(num_users=2, num_channels=5, num_episodes=10000):
    ...
    for episode in tqdm(range(num_episodes)):
        ...
        # Select channels for users using coordination approach
        actions = [0] * num_users
        available_channels = list(range(num_channels))
        if jammer_channel in available_channels:
            available_channels.remove(jammer_channel)
            
        for n in range(num_users):
            if available_channels:
                actions[n] = random.choice(available_channels)
                available_channels.remove(actions[n])
            else:
                actions[n] = random.randint(0, num_channels-1)
    ...
    for episode in tqdm(range(num_episodes)):
        ...
        for n in range(num_users):
            ...
            q_tables[n][state_key][actions[n]] = (1 - learning_rate) * old_q + \
                                                learning_rate * (rewards[n] + discount_factor * max_next_q)
...
def plot_time_frequency(actions_data, title, num_channels=5):
    ...
    for user_idx in range(len(actions_data[0][0])):
        ...
        plt.plot(user_channels, 'o-', label=f'User {user_idx+1}')
    
    # Plot jammer actions
    jammer_channels = [action[1] for action in actions_data]
    plt.plot(jammer_channels, 's-', label='Jammer', color='red')
...
def plot_normalized_rates(rates_cmaa, rates_sensing, rates_independent):
    ...
    plt.plot(x, rates_cmaa, 'b-', label='Proposed CMAA')
    plt.plot(x, rates_sensing, 'g--', label='Sensing based method')
    plt.plot(x, rates_independent, 'r-.', label='Independent Q-learning')
