# deep_q_learning_agent_pytorch_with_fh.py
...
class DeepQLearningAgent:
    ...
    def plot_progress(self, total_history, per_user_history, filename="fh_training_progress.png"):
        ...
        plt.title('Frequency Hopping Multi-User Training Progression')
        plt.legend()
        plt.grid(True)
    ...
    def train(self, save_path="fh_checkpoint.pth", plot_path="fh_training_progress.png"):
        total_reward = 0
        ...
        for i in range(T):
            ...
            if (i + 1) % step == 0:
                ...
                if (i + 1) == 100000:
                    self.plot_progress(total_history, per_user_history, "fh_progress_at_100k.png")
...
if __name__ == "__main__":
    agent = DeepQLearningAgent(load_path="fh_checkpoint.pth")
    avg_total_multi, avg_per_user_multi = agent.train(
        save_path="fh_checkpoint.pth",
        plot_path="fh_training_progress.png"
    )
    print(f"Frequency Hopping total average reward: {avg_total_multi:.4f}")
    print(f"Frequency Hopping per-user average rewards: {[f'{r:.4f}' for r in avg_per_user_multi]}")
    print(f"Frequency Hopping average per-user reward (mean): {np.mean(avg_per_user_multi):.4f}")